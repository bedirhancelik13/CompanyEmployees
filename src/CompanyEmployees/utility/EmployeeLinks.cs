...
namespace CompanyEmployees.utility
{
    public class EmployeeLinks : IEmployeeLinks
    {
        ...
        {
            if (!httpContext.Items.TryGetValue("AcceptHeaderMediaType", out var mediaTypeObj))
                return false;

            var mediaType = mediaTypeObj?.ToString();
            if (string.IsNullOrWhiteSpace(mediaType))
                return false;

            var typeWithoutSuffix = mediaType.Split('+')[0];

            return typeWithoutSuffix.EndsWith("hateoas", StringComparison.InvariantCultureIgnoreCase);
        }
        ...
        private List<Link> CreateLinksForEmployee(HttpContext httpContext, Guid companyId, Guid id, string fields = "")
        {
            ...
            {
                new Link(_linkGenerator.GetUriByAction(httpContext, action: "GetEmployeeForCompany",
                    values: new { companyId, id, fields }),
                    "self", "GET"),
                new Link(_linkGenerator.GetUriByAction(httpContext, action: "DeleteEmployeeForCompany",
                    values: new { companyId, id }),
                    "delete_employee", "DELETE"),
                new Link(_linkGenerator.GetUriByAction(httpContext, action: "UpdateEmployeeForCompany",
                    values: new { companyId, id }),
                    "update_employee", "PUT"),
                new Link(_linkGenerator.GetUriByAction(httpContext, action: "PartiallyUpdateEmployeeForCompany",
                    values: new { companyId, id }),
            ...
            LinkCollectionWrapper<Entity> employeesWrapper)
        {
            employeesWrapper.Links.Add(
                new Link(_linkGenerator.GetUriByAction(httpContext, "GetEmployeesForCompany", values: new { }),
                "self", "GET"));
